...
object CryptoManager {
    ...
    fun getPassphrase(context: Context): String {
        ...
        return if (encryptedPassphraseBase64 != null && ivBase64 != null) {
            runCatching {
                ...
                Log.w(TAG, "Stored database passphrase is no longer readable. Resetting local encryption state.", error)
                resetLocalEncryptionState(context)
    ...
    fun resetLocalEncryptionState(context: Context) {
        deleteStoredKey()
    ...
    fun nukeEverything(context: Context) {
        resetLocalEncryptionState(context)
    ...
    private fun createAndStorePassphrase(prefs: android.content.SharedPreferences): String {
    ...
    private fun deleteStoredKey() {
        runCatching {
            ...
            Log.w(TAG, "Could not clear the stored database key.", error)
        }
    }
}
